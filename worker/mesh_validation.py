...
def format_mesh_report(mesh_report: dict) -> str:
    ...
    def fmt(value, spec: str = "") -> str:
        # ponytail: la versione precedente faceva
        # f"{metrics.get('cells', 'N/A'):,}" - il format spec ":," (separatore
        # delle migliaia) applicato alla stringa di fallback "N/A" fa
        # esplodere con ValueError("Cannot specify ',' with 's'") non appena
        # una metrica manca (caso reale: un mesh_report con solo "cells",
        # vedi test_format_invalid_mesh_report). Qui si formatta solo se il
        # valore e' davvero numerico, altrimenti si mostra "N/A" senza
        # tentare di applicare alcuno spec.
    ...
    if metrics:
        lines.append("\nMetrics:")
        lines.append(f"  Cells:        {fmt(metrics.get('cells'))}")
        lines.append(f"  Points:       {fmt(metrics.get('points'))}")
        lines.append(f"  Skewness:     {fmt(metrics.get('max_skewness'), '.2f')}")
        lines.append(f"  Non-Ortho:    {fmt(metrics.get('max_non_orthogonality'), '.1f')}°")
        lines.append(f"  Aspect Ratio: {fmt(metrics.get('max_aspect_ratio'), '.0f')}")
