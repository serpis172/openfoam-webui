...
export function GeometryUpload({ projectId }: GeometryUploadProps) {
  ...
  const setGeometryFile = useWizardStore(s => s.setGeometryFile)
  const [files, setFiles] = useState<File[]>([])
  const [uploading, setUploading] = useState(false)

  const onDrop = useCallback((acceptedFiles: File[]) => {
    setFiles((prev) => [...prev, ...acceptedFiles])
    // ponytail: il preview 3D legge da qui, non aspetta l'upload -
    // risposta istantanea, il round-trip al server serve solo per
    // rendere il file disponibile al meshing, non per vederlo.
    const stl = acceptedFiles.find(f => f.name.toLowerCase().endsWith('.stl'))
    if (stl) setGeometryFile(stl)
  }, [setGeometryFile])
